/*
 * mergeTFileServiceHistogramsKB.cpp
 *
...
void make(TDirectory & out, TObject * o) {
  ...
  if((dir = dynamic_cast<TDirectory*>(o)) != 0) {
    ...
    h->Sumw2();
    h->SetDirectory(&out);
  } else if((th1i = dynamic_cast<TH1I*>(o)) != 0) {
    TH1I *h = (TH1I*) th1i->Clone();
    h->Reset();
    h->Sumw2();
    h->SetDirectory(&out);
    ...
    h->Sumw2();
    h->SetDirectory(&out);
  } else if((th2i = dynamic_cast<TH2I*>(o)) != 0) {
    TH2I *h = (TH2I*) th2i->Clone();
    h->Reset();
    h->Sumw2();
    h->SetDirectory(&out);
  }
...
void fill(TDirectory & out, TObject * o, double w) {
  ...
  if((dir  = dynamic_cast<TDirectory*>(o)) != 0) {
    ...
    const char * name = th1i->GetName();
    TH1I * outTh1i = dynamic_cast<TH1I*>(out.Get(name));
    if(outTh1i == 0) {
      cerr <<"error: histogram TH1D" << name << " not found in directory " << out.GetName() << endl;
      exit(-1);
    }
    ...
    const char * name = th2i->GetName();
    TH2I * outTh2i = dynamic_cast<TH2I*>(out.Get(name));
    if(outTh2i == 0) {
