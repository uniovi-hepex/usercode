...
//object definition
class MuCorrelatorTimingAnalyzer : public edm::EDAnalyzer {
public:

  //constructor, function is called when new object is created
  explicit MuCorrelatorTimingAnalyzer(const edm::ParameterSet& conf);

  //destructor, function is called when object is destroyed
  ~MuCorrelatorTimingAnalyzer();
...
MuCorrelatorTimingAnalyzer::MuCorrelatorTimingAnalyzer(const edm::ParameterSet& conf)
...
MuCorrelatorTimingAnalyzer::~MuCorrelatorTimingAnalyzer()
{ 
  cout <<" MuCorrelatorTimingAnalyzer end" << endl;
}

void MuCorrelatorTimingAnalyzer::beginJob()
{
  ...
  edm::LogImportant("MuCorrelatorTimingAnalyzer")<< "MuCorrelatorTimingAnalyzer::"<<__FUNCTION__<<":"<<__LINE__ << endl;
}

void MuCorrelatorTimingAnalyzer::endJob()
{
  cout << "MuCorrelatorTimingAnalyzer::"<<__FUNCTION__<<":"<<__LINE__ << endl;
}

void MuCorrelatorTimingAnalyzer::analyze(
    const edm::Event& event, const edm::EventSetup& es)
{
  const unsigned int omtflayersCnt = muCorrConfig.layers();
  ...
  //cout << "MuCorrelatorTimingAnalyzer::"<<__FUNCTION__<<":"<<__LINE__ <<" omtfCands->size(bxNumber) "<<omtfCands->size()<< endl;
  ...
  for (unsigned int iTP = 0; iTP < trackingParticleHandle->size(); ++iTP) {
    ...
    for(auto& matchedTTTrack : matchedTracks) {
      ...
      if (tpOfMatchedTTTrack.isNull()) { //if the ttTrack is at least loose genuine this cannot happened
        ...
        for(l1t::RegionalMuonCandBxCollection::const_iterator itOmtfCand = omtfCands->begin(bxNumber);
          ...
          if(omtfTtTrackPtr.get() == matchedTTTrack.get() ) { //it should be only one omtfTrack for given ttTRack
            LogTrace("l1tMuBayesEventPrint")<<" omtf track: "
                <<" quality  "<<itOmtfCand->hwQual()
                <<" pdfSum "<<(int)itOmtfCand->trackAddress().at(2)
                <<" "<<std::bitset<omtflayersCnt>((int)itOmtfCand->trackAddress().at(0))<<endl;

            if(ptOfBestOmtfCand < omtfTtTrackPtr->getMomentum(L1Tk_nPar).perp() ) {
              if(itOmtfCand->hwQual() >= muCandQualityCut) {
                ptOfBestOmtfCand = omtfTtTrackPtr->getMomentum(L1Tk_nPar).perp();
                itBestOmtfCand = itOmtfCand;
                //LogTrace("l1tMuBayesEventPrint")<<" itBestOmtfCand set"<<endl;
              }
            }
            break;
          }
    ...
    if(itBestOmtfCand != l1Omtf.product()->end(bxNumber) ) {
      ...
      std::bitset<omtflayersCnt> layerHitBits(layerHits);
      ...
      //if (DebugMode)
      {
        ...
        int omtfTTTrackIndex = (int)itBestOmtfCand->trackAddress().at(3);
        LogTrace("l1tMuBayesEventPrint")<<"\nbest omtf track: omtfTTTrackIndex "<<omtfTTTrackIndex<<" pt "<<itBestOmtfCand->hwPt()<<" = "<<(itBestOmtfCand->hwPt()-1)/2.<<" GeV "
            <<" eta "<<muCorrConfig.hwEtaToEta(itBestOmtfCand->hwEta())
        <<" phi "<<muCorrConfig.hwPhiToGlobalPhi(itBestOmtfCand->hwPhi() )
      ...
      if(nMatch) {
        ...
        if(tpPtr->pt() > 20)
        {
          ...
          edm::LogImportant("l1tMuBayesEventPrint")<<" no omtf candidate. omtfCands->size(): "<<omtfCands->size()<<(tpPtr->pt() > 10 ? " lost high Pt" : "")<<endl<<endl;
        }
      }
    }
   ...
   */
  //we one should find one candidate with max pt and use it for filling the histograms
  auto itBestOmtfCand = muCorrTracks->end(bxNumber);
  float bestPt = 0;
//  for(l1t::RegionalMuonCandBxCollection::const_iterator itOmtfCand = omtfCands->begin(bxNumber);
//      itOmtfCand != l1Omtf.product()->end(bxNumber); ++itOmtfCand) {

  for(auto itOmtfCand = muCorrTracks->begin(bxNumber);
        itOmtfCand != muCorrTracks->end(bxNumber); ++itOmtfCand) {

    //int omtfTTTrackIndex = (int)itOmtfCand->trackAddress().at(3);
    //edm::Ptr< TTTrack< Ref_Phase2TrackerDigi_ > > ttTrackPtr(TTTrackHandle, omtfTTTrackIndex);

    auto ttTrackPtr = itOmtfCand->getTtTrackPtr();
    if(ttTrackPtr.isNull() ) {
      edm::LogImportant("l1tMuBayesEventPrint")<<__FUNCTION__<<":"<<__LINE__<<"ttTrackPtr.isNull() !!!!!!!!!!!!!"<<endl;
      continue;
    }
    ...
    int quality = itOmtfCand->hwQual();
    if(quality < muCandQualityCut)
      continue;

    if(abs(ttTrackPtr->getMomentum(L1Tk_nPar).eta()) >= etaCutFrom && abs(ttTrackPtr->getMomentum(L1Tk_nPar).eta()) <= etaCutTo) {
    }
    else
      continue;
    /*
  ...
  if(itBestOmtfCand != muCorrTracks->end(bxNumber))  {
    //int refLayer = (int)it->trackAddress().at(1);
    auto ttTrackPtr = itBestOmtfCand->getTtTrackPtr();
    ...
    if(tpMatchedToOmtfTTTrack.isNull() || abs(tpMatchedToOmtfTTTrack->pdgId()) != 13) {
      auto& layerHitBits = itBestOmtfCand->getFiredLayerBits();
      int firedLayers = layerHitBits.count();
      int pdfSum = (int)itBestOmtfCand->pdfSum();
      ...
      {
        edm::LogImportant("l1tMuBayesEventPrint")<<"\nrun:lumi:event "<<event.run()<<":"<<event.luminosityBlock()<<":"<<event.id().event()<<endl;
        edm::LogImportant("l1tMuBayesEventPrint")<<"fake omtf track: pt "<<itBestOmtfCand->hwPt()<<" = "<<(itBestOmtfCand->hwPt()-1)/2.<<" GeV "
            <<" eta "<<muCorrConfig.hwEtaToEta(itBestOmtfCand->hwEtaAtVtx())
        <<" phi "<<muCorrConfig.hwPhiToGlobalPhi(itBestOmtfCand->hwPhiAtVtx() )
        <<" hwSign "<<itBestOmtfCand->hwSign()
...
DEFINE_FWK_MODULE(MuCorrelatorTimingAnalyzer);
