...
private:
  ...
  void hscpAnalysis(edm::Ptr< TrackingParticle >& trackPartPtr, edm::Handle<l1t::BayesMuCorrTrackBxCollection>& muCorrTracksHandle);

  void analyseMuonsInNextBXes(edm::Handle< std::vector< TrackingParticle > >& trackingParticleHandl, bool cancelledByTriggerRules);
  ...
  TH1D* hscpGenEta = nullptr;
  TH1D* hscpGenPt = nullptr;
...
: parameterSet(conf), eventCount(0)
{
  ...
  inputMuCorr = consumes<l1t::BayesMuCorrTrackBxCollection>(edm::InputTag("simBayesMuCorrelatorTrackProducer", "BayesMuCorrTracks"));
...
void MuCorrelatorAnalyzer::beginJob()
{
  ...
  hscpGenEta = fs->make<TH1D>("hscpGenEta", "hscpGenEta; eta; #events", etaBins, -3, 3);
  hscpGenPt  = fs->make<TH1D>("hscpGenPt", "hscpGenPt; ptGen [GeV]; #events", 100, 0, 1000);

  etaGenPtGenBx1 = fs->make<TH2I>("etaGenPtGenBx1", "etaGenPtGenBx1; eta; pt [GeV]; ", 50, -2.4, 2.4, 50, 0, 100);
  etaGenPtGenLostBx1 = fs->make<TH2I>("etaGenPtGenLostBx1", "etaGenPtGenLostBx1; eta; pt [GeV]; ", 50, -2.4, 2.4, 50, 0, 100);

  etaGenPtGenBx2 = fs->make<TH2I>("etaGenPtGenBx2", "etaGenPtGenBx2; eta; pt [GeV]; ", 50, -2.4, 2.4, 50, 0, 100);
  etaGenPtGenLostBx2 = fs->make<TH2I>("etaGenPtGenLostBx2", "etaGenPtGenLostBx2; eta; pt [GeV]; ", 50, -2.4, 2.4, 50, 0, 100);
...
void MuCorrelatorAnalyzer::analyzeSimTracks(edm::Handle<edm::SimTrackContainer>& simTraksHandle, edm::Handle<l1t::BayesMuCorrTrackBxCollection>& muCorrTracksHandle) {
  ...
  for (unsigned int iSimTrack = 0; iSimTrack != simTraksHandle->size(); iSimTrack++ ) {
    ...
    if ( (abs(simTrackPtr->type()) == 13  ||  abs(simTrackPtr->type()) == 1000015) && simTrackPtr->momentum().pt() > 10) { //TODO 1000015 is stau
      LogTrace("l1tMuBayesEventPrint")<<__FUNCTION__<<":"<<__LINE__<<" sim.type() "<<simTrackPtr->type()<<" genpartIndex "<<simTrackPtr->genpartIndex()
              <<" Beta() "<<simTrackPtr->momentum().Beta()<<std::endl;

      if (abs(simTrackPtr->type()) == 1000015) {
          hscpGenEta->Fill(simTrackPtr->momentum().eta());
          hscpGenPt->Fill(simTrackPtr->momentum().pt());
      }

      if(abs(simTrackPtr->momentum().eta()) < etaCutFrom || abs(simTrackPtr->momentum().eta()) > etaCutTo)
         continue;

      bool wasL1MuCand = false;
      for(auto itL1MuCand = muCorrTracks->begin(bxNumber); itL1MuCand != muCorrTracks->end(bxNumber); ++itL1MuCand) {

        auto l1MuCandsimTrackPtr = itL1MuCand->getSimTrackPtr();
        if(l1MuCandsimTrackPtr.isNull() ) {
          edm::LogImportant("l1tMuBayesEventPrint")<<__FUNCTION__<<":"<<__LINE__<<" simTrackPtr.isNull() !!!!!!!!!!!!!"<<endl;
          continue;
        }

        if(l1MuCandsimTrackPtr != simTrackPtr)
          continue;
        ...
        if(l1MuBeta == 1)
          l1MuBeta = 0.99;
        betaGenBetaL1Mu->Fill(l1MuCandsimTrackPtr->momentum().Beta(), l1MuBeta);
        wasL1MuCand = true;
      }

      if(!wasL1MuCand) {
        LogTrace("l1tMuBayesEventPrint")<<__FUNCTION__<<":"<<__LINE__<<" no L1MuCand !!!!!!!!!!!!!"<<std::endl;
        betaGenBetaL1Mu->Fill(simTrackPtr->momentum().Beta(), -1);
      }
    }
  }
}
...
void MuCorrelatorAnalyzer::hscpAnalysis(edm::Ptr< TrackingParticle >& trackPartPtr, edm::Handle<l1t::BayesMuCorrTrackBxCollection>& muCorrTracksHandle) {
  auto muCorrTracks = muCorrTracksHandle.product();
  int bxNumber = 0;

  if(abs(trackPartPtr->eta()) < etaCutFrom || abs(trackPartPtr->eta()) > etaCutTo)
    return;

  if ( (abs(trackPartPtr->pdgId()) == 13  ||  abs(trackPartPtr->pdgId()) == 1000015) && trackPartPtr->pt() > 10) { //TODO 1000015 is stau
    ...
    for(auto itL1MuCand = muCorrTracks->begin(bxNumber); itL1MuCand != muCorrTracks->end(bxNumber); ++itL1MuCand) {
      ...
      if(l1MuCandSimTrackPtr.isNull() ) {
        //edm::LogImportant("l1tMuBayesEventPrint")<<__FUNCTION__<<":"<<__LINE__<<" trackPartPtr.isNull() !!!!!!!!!!!!!"<<endl;
        continue;
      }

      if(l1MuCandSimTrackPtr != trackPartPtr)
        continue;
...
void MuCorrelatorAnalyzer::analyseMuonsInNextBXes(edm::Handle< std::vector< TrackingParticle > >& trackingParticleHandle, bool cancelledByTriggerRules) {
  for (unsigned int iTrackPart = 0; iTrackPart != trackingParticleHandle->size(); iTrackPart++ ) {
    edm::Ptr< TrackingParticle > trackPartPtr(trackingParticleHandle, iTrackPart);
    ...
    if ( (abs(trackPartPtr->pdgId()) == 13  ||  abs(trackPartPtr->pdgId()) == 1000015) ) { //TODO 1000015 is stau
      if(trackPartPtr->eventId().bunchCrossing() == 1) {
        etaGenPtGenBx1->Fill(trackPartPtr->eta(), trackPartPtr->pt() );
        if(cancelledByTriggerRules) {
          etaGenPtGenLostBx1->Fill(trackPartPtr->eta(), trackPartPtr->pt() );

          if(trackPartPtr->pt() > 10)
            edm::LogImportant("l1tMuBayesEventPrint")<<__FUNCTION__<<":"<<__LINE__<<" lost high pt muon in BX+1, pt:  "<<trackPartPtr->pt()<<std::endl;
        }
      }

      if(trackPartPtr->eventId().bunchCrossing() == 2) {
        etaGenPtGenBx2->Fill(trackPartPtr->eta(), trackPartPtr->pt() );
        if(cancelledByTriggerRules) {
          etaGenPtGenLostBx2->Fill(trackPartPtr->eta(), trackPartPtr->pt() );
          if(trackPartPtr->pt() > 10)
            edm::LogImportant("l1tMuBayesEventPrint")<<__FUNCTION__<<":"<<__LINE__<<" lost high pt muon in BX+2, pt:  "<<trackPartPtr->pt()<<std::endl;
        }
      }
    ...
    const edm::Event& event, const edm::EventSetup& es)
{
  ...
  for (unsigned int iTP = 0; iTP < trackingParticleHandle->size(); ++iTP) {
    ...
    hscpAnalysis(tpPtr, muCorrTracksHandle);
    ...
    //l1t::RegionalMuonCandBxCollection::const_iterator itBestL1MuCand = l1Omtf.product()->end(bxNumber);
    auto itBestL1MuCand = muCorrTracks->end(bxNumber);
    float ptOfBestL1MuCand = 0;

    // ----------------------------------------------------------------------------------------------
    // loop over ttRracks matched to this tracking particles
    ...
    for(auto& matchedTTTrack : matchedTracks) {
      ...
      if (tpOfMatchedTTTrack.isNull()) { //if the ttTrack is at least loose genuine this cannot happened
        ...
        //we one should find one candidate with max pt and use it for filling the histograms
        for(auto itL1MuCand = muCorrTracks->begin(bxNumber); itL1MuCand != muCorrTracks->end(bxNumber); ++itL1MuCand) {
          auto& omtfTtTrackPtr = itL1MuCand->getTtTrackPtr();
          //edm::Ptr< TTTrack< Ref_Phase2TrackerDigi_ > > omtfTtTrackPtr(TTTrackHandle, omtfTTTrackIndex);

          if(omtfTtTrackPtr.isNonnull() && omtfTtTrackPtr.get() == matchedTTTrack.get() ) { //it should be only one omtfTrack for given ttTRack
            LogTrace("l1tMuBayesEventPrint")<<" omtf track: "
                <<" quality  "<<itL1MuCand->hwQual()
                <<" pdfSum "<<(int)itL1MuCand->pdfSum()
                <<" "<<itL1MuCand->getFiredLayerBits()<<endl;

            if(ptOfBestL1MuCand < omtfTtTrackPtr->getMomentum(L1Tk_nPar).perp() ) {
              if(itL1MuCand->hwQual() >= muCandQualityCut) {
                ptOfBestL1MuCand = omtfTtTrackPtr->getMomentum(L1Tk_nPar).perp();
                itBestL1MuCand = itL1MuCand;
    ...
    //filling OMTF histograms, at most only one candidate for every tracking particle
    if(itBestL1MuCand != muCorrTracks->end(bxNumber) && (!applyTriggerRules || (applyTriggerRules && itBestL1MuCand->getCanceledByTriggerRules() == 0) ) ) {
      /* LogTrace("l1tMuBayesEventPrint")<<" itL1MuCand->trackAddress().size "<<itL1MuCand->trackAddress().size()<<endl;
          LogTrace("l1tMuBayesEventPrint")<<" itL1MuCand->trackAddress().at(0) "<<itL1MuCand->trackAddress().at(0)<<endl;
          LogTrace("l1tMuBayesEventPrint")<<" itL1MuCand->trackAddress().at(1) "<<itL1MuCand->trackAddress().at(1)<<endl;
          LogTrace("l1tMuBayesEventPrint")<<" itL1MuCand->trackAddress().at(2) "<<itL1MuCand->trackAddress().at(2)<<endl;
          LogTrace("l1tMuBayesEventPrint")<<" itL1MuCand->trackAddress().at(3) "<<itL1MuCand->trackAddress().at(3)<<endl;*/

      auto& l1track_ptr = itBestL1MuCand->getTtTrackPtr(); //itBestL1MuCand cannot be null here
      ...
      auto& layerHitBits = itBestL1MuCand->getFiredLayerBits();
      int firedLayers = layerHitBits.count();
      int pdfSum = itBestL1MuCand->pdfSum();
      if(ptTTTrackOmtf >= 18)
      ...
      //if (DebugMode)
      {
        ...
        LogTrace("l1tMuBayesEventPrint")<<"\nbest omtf track: "<<" pt "<<itBestL1MuCand->hwPt()<<" = "<<(itBestL1MuCand->hwPt()-1)/2.<<" GeV "
            <<" eta "<<muCorrConfig.hwEtaToEta(itBestL1MuCand->hwEtaAtVtx() )
        <<" phi "<<muCorrConfig.hwPhiToGlobalPhi(itBestL1MuCand->hwPhiAtVtx() )
        <<" hwSign "<<itBestL1MuCand->hwSign()
      ...
      if(nMatch) {
        ...
        if(tpPtr->pt() > 10)
        {
          ...
          if(itBestL1MuCand == muCorrTracks->end(bxNumber)) {
            edm::LogImportant("l1tMuBayesEventPrint")<<" no correlator candidate. omtfCands->size(): "<<omtfCands->size()<<(tpPtr->pt() > 10 ? " lost high Pt" : "")<<endl<<endl;
          }
          else {

            edm::LogImportant("l1tMuBayesEventPrint")<<" correlator candidate cancelled by trigger rules: "<<endl;
            edm::LogImportant("l1tMuBayesEventPrint")<<printTrackigParticle();
            for(int bx = muCorrTracks->getFirstBX(); bx <= muCorrTracks->getLastBX(); bx++) {
              for(auto itL1MuCand = muCorrTracks->begin(bx); itL1MuCand != muCorrTracks->end(bx); ++itL1MuCand) {
                edm::LogImportant("l1tMuBayesEventPrint")<<" bx "<<bx<<" pt "<<itL1MuCand->getPt()<<" CanceledByTriggerRules: "<<itL1MuCand->getCanceledByTriggerRules()<<endl;
              }
            }
            edm::LogImportant("l1tMuBayesEventPrint")<<endl;
          }
  ...
  auto itBestL1MuCand = muCorrTracks->end(bxNumber);
  float bestPt = 0;
//  for(l1t::RegionalMuonCandBxCollection::const_iterator itL1MuCand = omtfCands->begin(bxNumber);
//      itL1MuCand != l1Omtf.product()->end(bxNumber); ++itL1MuCand) {
  ...
  for(auto itL1MuCand = muCorrTracks->begin(bxNumber);
    ...
    //assuming that the double mu algorithm is without any cut, but with the cut on the ttTrackNstub
     ...
     */
    if(bestPt < itL1MuCand->getPt() ) {
      bestPt = itL1MuCand->getPt();
      itBestL1MuCand = itL1MuCand;
    }
  ...
  if(itBestL1MuCand != muCorrTracks->end(bxNumber))  {
    //int refLayer = (int)it->trackAddress().at(1);
    auto& ttTrackPtr = itBestL1MuCand->getTtTrackPtr();
    ...
    if(tpMatchedToOmtfTTTrack.isNull() || abs(tpMatchedToOmtfTTTrack->pdgId()) != 13) {
      auto& layerHitBits = itBestL1MuCand->getFiredLayerBits();
      int firedLayers = layerHitBits.count();
      int pdfSum = (int)itBestL1MuCand->pdfSum();
      ...
      {
        edm::LogImportant("l1tMuBayesEventPrint")<<"\nrun:lumi:event "<<event.run()<<":"<<event.luminosityBlock()<<":"<<event.id().event()<<endl;
        edm::LogImportant("l1tMuBayesEventPrint")<<"fake omtf track: pt "<<itBestL1MuCand->hwPt()<<" = "<<(itBestL1MuCand->hwPt()-1)/2.<<" GeV "
            <<" eta "<<muCorrConfig.hwEtaToEta(itBestL1MuCand->hwEtaAtVtx())
        <<" phi "<<muCorrConfig.hwPhiToGlobalPhi(itBestL1MuCand->hwPhiAtVtx() )
        <<" hwSign "<<itBestL1MuCand->hwSign()
        <<" pdfSum "<<pdfSum<<" firedLayers "<<firedLayers
        <<" "<<layerHitBits<<(itBestL1MuCand->hwPt() > 21 ? " fake high Pt" : "")<<endl;
  ...
  bool cancelledByTriggerRules = false;
  if(singleMuCands >= 1 || doubleMuCands >= 2)
    cancelledByTriggerRules = true;
  analyseMuonsInNextBXes(trackingParticleHandle, cancelledByTriggerRules);
